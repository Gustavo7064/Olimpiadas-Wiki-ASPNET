...
-- =====================================================
-- Este script recria o banco do zero (DROP + CREATE).
-- Rode ele inteiro de uma vez para substituir o banco antigo.
--
-- Correções feitas nesta versão:
-- 1) Provas de Ginástica agora são inseridas ANTES dos
--    resultados da Rebeca Andrade (evita que o codProva
--    auto-increment dela colida com IDs de Natação).
-- 2) Resultados da Rebeca (edições 23 e 24) apontam para
--    as provas certas de Ginástica.
-- 3) Removidos resultados fictícios das edições 26 (2028)
--    e 27 (2032), que ainda não ocorreram.
-- =====================================================

-- 1. Banco de Dados
DROP DATABASE IF EXISTS bdolimpicoJueGu;
...
-- 6.1 Provas de Ginástica (ID modalidade 5)
-- IMPORTANTE: precisa ser inserido ANTES dos resultados
-- da Rebeca Andrade (abaixo), senão o codProva auto-increment
-- dela colide com os IDs de Natação já inseridos.
INSERT INTO provas (prova, codModalidade) VALUES
...
-- CORRIGIDO: usava codProva 70/71 (que eram provas de Natação),
-- agora aponta para as provas certas de Ginástica pelo nome.
INSERT INTO resultadosatletas (codAtleta, codProva, edicao, resultado, medalha) VALUES
(9, (SELECT codProva FROM provas WHERE prova = 'Individual All-Around Feminino' AND codModalidade = 5), 23, '11ºLugar',''),
(9, (SELECT codProva FROM provas WHERE prova = 'Salto sobre a mesa Feminino' AND codModalidade = 5), 24, '1ºLugar','Ouro');
...
-- REMOVIDO: essa edição ainda não ocorreu (Jogos de 2028),
-- então não pode ter resultados/medalhas cadastrados.
-- Este bloco existia apenas como dado de teste e não deveria
-- ter ido para o banco "de produção".

-- Atletas para edição 27 (2032 - Brisbane)
-- REMOVIDO: essa edição ainda não ocorreu (Jogos de 2032),
-- então não pode ter resultados/medalhas cadastrados.
-- Este bloco existia apenas como dado de teste e não deveria
-- ter ido para o banco "de produção".
...
-- 12. Provas de Ginástica: já inseridas na seção 6.1 (removido daqui
-- para não duplicar e para não repetir o bug original de ordenação).
